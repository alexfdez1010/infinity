...
constexpr int CL_ITEM_COUNT  = 6;  // 0=cover, 1-5=grid cells
}  // namespace
...
void HomeActivity::loopClassic() {
  ...
  if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) {
    switch (selectorIndex) {
      ...
      case 4: onFileTransferOpen(); break;
...
void HomeActivity::renderClassic() {
  ...
  if (!coverRendered) {
    ...
    renderHeaderClock();
    // Grid lines. The bottom row (Settings) spans the full width, so the vertical
    // divider stops at the third row instead of continuing to the grid bottom.
    ...
    renderer.drawLine(CL_DIVIDER_X, CL_DIVIDER_Y,  CL_DIVIDER_X, CL_GRID_ROW3_Y, true);
  ...
  renderGridCell(CL_DIVIDER_X, CL_DIVIDER_Y,   cw,     rh1, 1, LibraryIcon,   tr(STR_BROWSE_FILES));
  renderGridCell(0,            CL_GRID_ROW2_Y, cw,     rh2, 2, RecentIcon,    tr(STR_MENU_RECENT_BOOKS));
  renderGridCell(CL_DIVIDER_X, CL_GRID_ROW2_Y, cw,     rh2, 3, TransferIcon,  tr(STR_FILE_TRANSFER));
  // Settings spans the full width of the bottom row (pet cell removed).
  renderGridCell(0,            CL_GRID_ROW3_Y, cw * 2, rh3, 4, Settings2Icon, tr(STR_SETTINGS_TITLE));
