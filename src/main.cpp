...
void setup() {
  ...
  CROSSPET_SETTINGS.loadFromFile();
  I18N.loadSettings();
  FontMgr.scanFonts();
  FontMgr.loadSettings();
  FontMgr.setUiExternalAllowed(CROSSPET_SETTINGS.systemWideCustomFont != 0);
  KOREADER_STORE.loadFromFile();
  READ_STATS.loadFromFile();   // loaded early so abort-to-sleep paths show correct stats
  GAMIFY.loadFromFile();       // after READ_STATS: first run seeds streak/records from it
  ...
  // Restore system clock after deep sleep. This device has no battery-backed RTC,
  // so the clock is reconstructed from persisted timestamps on every boot.
  ...
  //             written on every sleep entry via saveClockToSD(), with elapsed correction)
  // Strategy 3: Build-time fallback (last resort — at least a baseline date for reading stats)
  {
