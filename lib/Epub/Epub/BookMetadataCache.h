...
class BookMetadataCache {
 ...
 public:
  ...
  // Invalidate the in-memory cache: closes the backing files and marks the
  // cache unloaded so subsequent reads fail safe. Must be called whenever the
  // on-disk cache is deleted (e.g. Epub::clearCache), otherwise getSpineEntry()
  // would seek into a dead file handle and read a garbage-length string.
  void unload();
};
