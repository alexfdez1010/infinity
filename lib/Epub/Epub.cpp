...
bool Epub::clearCache() const {
  ...
  // The on-disk cache is gone; invalidate the in-memory metadata cache so any
  // subsequent read (e.g. saveProgress -> calculateProgress -> getSpineItem)
  // fails safe instead of seeking into a now-dead file handle and reading a
  // garbage-length string (which aborted with std::bad_alloc).
  if (bookMetadataCache) bookMetadataCache->unload();
