...
| Tool | Description |
|------|-------------|
| **Clock** | Display current time with date, useful as an always-on bedside clock mode. |
| **Weather** | Current conditions and 5-day forecast via Open-Meteo (no account needed). |
| **Pomodoro** | Pomodoro timer for productivity — cycles between work and break intervals. |
| **Reading Stats** | Session time, current/longest reading streaks, and per-book progress. |
| **Reading Goals** | Daily reading goal with streak and freeze tokens, plus an achievement badge list. |
| **Sleep Image** | Pick and manage custom sleep screen images from the SD card. |
| **Flashcards** | SM-2 spaced repetition flashcard decks, imported from the SD card. |
| **OPDS Browser** | Browse and download books from a configured Calibre/OPDS library (only shown once a server is configured). |
| **2048** | Slide tiles to combine matching numbers and reach the 2048 tile. |
| **Political Simulator** | Reigns-style card game — balance four powers as a satirical politician. |

Each tool can be individually shown or hidden via its toggle in **Settings**; Games (2048 and Political Simulator) share a single master toggle.
